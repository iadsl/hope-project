...
class Exp_Main(Exp_Basic):
    ...
    #New code added to choose cpu
    """def _move_to_device(self, x, device):
        #Safely move nested batches/tensors to device (CPU or CUDA).
        if isinstance(x, dict):
            return {k: self._move_to_device(v, device) for k, v in x.items()}
        if isinstance(x, (list, tuple)):
            t = [self._move_to_device(v, device) for v in x]
            # preserve tuple type
                ...
                for i, batch in enumerate(vali_loader):
                    ...
                    if "y_mask" in batch.keys():
                        if torch.sum(batch["y_mask"]).item() == 0:
                            ...
                            continue
                    #Commenting this out to choose cpu        
    ...
    def train(self) -> None:
        ...
        #Commented the next 2 lines and using CPU instead
        if not self.configs.use_multi_gpu:
            model_train = model_train.to(f"cuda:{self.configs.gpu_id}")
        """device = None
        if torch.cuda.is_available() and getattr(self.configs, "use_gpu", True):
        # map gpu_id safely into available devices
            gpu_id = getattr(self.configs, "gpu_id", 0)
            ...
            for epoch in tqdm(range(self.configs.train_epochs), desc="Epochs"):
                ...
                with tqdm(total=len(train_loader), leave=False, desc="Training") as it:
                    ...
                    for i, batch in enumerate(train_loader):
                        ...
                        #Commented the following line to choose cpu
                        if not self.configs.use_multi_gpu:
                            batch = {k: v.to(f"cuda:{self.configs.gpu_id}") for k, v in batch.items()}
                        #if not self.configs.use_multi_gpu:
                         #   batch = self._move_to_device(batch, self.device)
