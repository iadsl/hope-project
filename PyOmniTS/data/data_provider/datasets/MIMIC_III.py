...
from data.dependencies.tsdm.tasks.mimic_iii_debrouwer2019 import Sample
...
#Commented the following block to take the preprocessed file from my path
    """def preprocess(self):
        
        preprocess without time alignment
        
        if self.seq_len + self.pred_len > 96:
            logger.exception(f"{self.seq_len+self.pred_len=} is too large. Expect the value smaller than 96")
            exit(1)
            ...
            for sample in dataset:
                ...
                if self.configs.collate_fn == "collate_fn_patch":
                    ...
                    for i in range(n_patch):
                        observations = x_mark < ((i + 1) * PATCH_LEN / 96)
                    ...
                    for j in range(n_patch_y):
                        observations = y_mark < (((n_patch + j + 1) * PATCH_LEN) / 96)
    ...
    def preprocess(self):
        """
        preprocess without time alignment using custom data path
        """
        import os
        ...
        # Set the custom data path (where your complete_tensor.csv is located)
        custom_data_path = "/mnt/projects/zhuangyo_project/hope_project_data/mimic/mimic-iii/physionet.org/files/mimiciii/1.4/processed"
        
        # Override the environment variable and dataset path
        os.environ['TSDM_RAWDATA_DIR'] = custom_data_path
        
        # Direct path override
        from data.dependencies.tsdm.datasets.mimic_iii_debrouwer2019 import MIMIC_III_DeBrouwer2019 as DatasetMIMIC
        DatasetMIMIC.RAWDATA_DIR = Path(custom_data_path)
        
        try:
            # Now create the task with the overridden path
            task = MIMIC_III_DeBrouwer2019(
                seq_len=self.seq_len - 0.5,
                pred_len=self.pred_len
            )
        except Exception as e:
            logger.error(f"Failed to initialize MIMIC task with custom path: {e}")
            logger.info(f"Make sure complete_tensor.csv exists at: {custom_data_path}/complete_tensor.csv")
            raise
...
def collate_fn_patch(
    ...
    '''
    time-aligned patching (constant number of time steps)
    '''
    global configs
    seq_len_max_irr: int = configs.seq_len_max_irr
